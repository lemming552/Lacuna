...
sub send_excavators {
    ...
    for my $planet (keys %{$status->{planets}}) {
        ...
        if ($status->{ready}{$planet} and @{$status->{ready}{$planet}}) {
            ...
            #########################FIX for max excavator count
            my $excavatorinfo = $status->{archmin}{$planet}->view_excavators();
            my @excavators = @{ $excavatorinfo->{excavators} };
            my $exctraveling = $excavatorinfo->{travelling};
            my $excavatorcount = (@excavators-1)+$exctraveling;
            my $maxexcavators = $excavatorinfo->{max_excavators};
            printf "$planet has $excavatorcount excavators and a max of $maxexcavators\n";
            if ($excavatorcount == $maxexcavators) {next PLANET;}
            ##############################################
                ...
                while (!$all_done) {
                    ...
                    for (@dests) {
                        ...
                        if ($opts{'dry-run'}) {
                            ...
                            output("Sending excavator $ex->{id} from $planet to $dest_name (" . pluralize($distance, "unit") . ", zone $zone)...\n");
                            my $launch_status;
                            my $ok = eval {
                                ####$launch_status = $port->send_ship($ex->{id}, {x => $x, y => $y});
                                #########################################
                                $launch_status = $port->send_ship($ex->{id}, {body_name => $dest_name});
                                $excavatorcount++;
                                if ($excavatorcount == $maxexcavators) {next PLANET;}
                                #########################################
                                return 1;
