...
# Load planet data
  ...
  do {
    ...
    my $build_time = 0;
    unless ($queue_ignore) {
      $build_time = build_time( $buildings );
    }
    
    if ( $build_time ) {
      $build_time += 5;
      $build_time = 60 if ($build_time > 900 or $build_time < 0);
...
sub plan_count {
  ...
  if (scalar @halls > 1) {
    print "More than one set of halls found! Using first set.\n";
  }
    
  return $halls[0]->{quantity};
}

sub build_time {
  my ( $buildings ) = @_;
    
  return max grep { defined }
    map {
...
Usage: $0 CONFIG_FILE
    ...
    --ignore_queue  Don't wait, just build to max allowed by dev min and quit
