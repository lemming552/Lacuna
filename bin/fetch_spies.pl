...
# reverse hash, to key by name instead of id
  ...
  die "No space port found on $opts{dest}\n" unless $space_port_id;
  die "No space port found on $opts{dest}\n" unless $intel_id;

  print "Getting spies and ships from $opts{dest}\n";
  ...
  my $prep;
  my $ok = eval {
    $prep = $space_port->prepare_fetch_spies( $from_id, $dest_id);
  };
  if ($ok) {
#    @spies = @{$prep->{spies}};
    @ships = @{$prep->{ships}};
  ...
  unless ($ship) {
    ...
    print "Fetching ",scalar @spy_ids," from $opts{from} to $opts{dest} using ",
        $ship->{name}, "\n";
    my $sent;
    my $ok = eval {
        $sent = $space_port->fetch_spies( $from_id, $dest_id, $ship->{id}, \@spy_ids);
      };
    if ($ok) {
      print "Spies fetched, arriving ", $sent->{ship}->{date_arrives};
    }
    else {
      my $error = $@;
      print $error,"\n";
